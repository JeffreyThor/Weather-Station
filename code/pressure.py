...
data = {}
pressureList = []
with open('/home/pi/Desktop/pressureData.json','r') as infile:
        data = json.load(infile)
# pressureList.append(data)
# data['time'] = time.strftime('%m/%d/%Y %H:%M:%S%z')
# data['reading' + str(time.time())] = str(pressure+p_decimal)+" Pa" + " at time: " + time.strftime('%m/%d/%Y %H:%M:%S%z')
a_dict = {'reading' + str(time.time()): str(pressure+p_decimal) +" Pa" + " at time: " + time.strftime('%m/%d/%Y %H:%M:%S%z')} 
data.update(a_dict)
# data['celsius'] = str(celsius)+deg+"C"
# data['fehrenheit'] = str(fahrenheit)+deg+"F"
#pressureList.append(data)
# json_data = json.dumps(data)

with open('/home/pi/Desktop/pressureData.json', 'w') as outfile:
